...
While a Promie object is "pending" (working), the result is undefined.
    ...
    return new Promise((resolve, reject) => {
        setTimeout(() => {
            if (songName.length > 5){
                ...
                reject(`${songName} does not exist in the database`)
            }
        }, 3000)
    })
}
...
Using a chain of callbacks and .then/.catch/.finally we can resolve the promise and receive its output
...
/*
Awync Await is another way to resolve your promises, that is a bit more clear than the .then.catch chain
To do this we use the key word await in front of the promise call
any function that is awaiting something must be labled as async.
